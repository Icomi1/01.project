...
void JustFloat_send(float pitch,float roll,float yaw)   //justfloat 数据协议传输 pitch,roll,yaw
{
   u8 byte[4]={0};      //float转化为4个字节数据
   u8 tail[4]={0x00, 0x00, 0x80, 0x7f};   //帧尾

    //依次向上位机发送数据
   Float_to_Byte(pitch,byte);
   Serial_SendArray(byte);   
   
    Float_to_Byte(roll,byte);
   Serial_SendArray(byte);   
    
    Float_to_Byte(yaw,byte);
   Serial_SendArray(byte);   
   
    //发送帧尾
   Serial_SendArray(tail);   //帧尾为 0x00 0x00 0x80 0x7f
